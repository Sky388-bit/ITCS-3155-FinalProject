...
def read_all(menu_id: int = None, db: Session = Depends(get_db)):
    return controller.read_all(db, menu_id=menu_id)
...
def update(item_id: int, request: schema.RatingsUpdate, db: Session = Depends(get_db)):
